...
function regionClass(regions, key) {
  return `muscle-map__region is-${regions[key].tier}`;
}
...
function regionTitle(regions, key, label) {
  ...
  return `${label}: ${tierLabel}, ${formatCount(region.score)} weighted set score.`;
}

function MuscleBodyMap({ regions }) {
  return (
    <div className="muscle-map">
      ...
      >
        ...
        <desc id="muscle-map-description">
          A front and back body diagram. Grey indicates no exposure, sage indicates light exposure, butter indicates moderate exposure, and coral indicates high exposure from completed mapped sets.
        </desc>
        ...
        <g className="muscle-map__figure muscle-map__figure--front">
          ...
          <g className={regionClass(regions, "shoulders")}>
          ...
          <g className={regionClass(regions, "chest")}>
          ...
          <g className={regionClass(regions, "biceps")}>
          ...
          <g className={regionClass(regions, "core")}>
          ...
          <g className={regionClass(regions, "hipFlexors")}>
          ...
          <g className={regionClass(regions, "quads")}>
          ...
          <g className={regionClass(regions, "lowerLegs")}>
        ...
        <g className="muscle-map__figure muscle-map__figure--back">
          ...
          <g className={regionClass(regions, "upperBack")}>
          ...
          <g className={regionClass(regions, "lats")}>
          ...
          <g className={regionClass(regions, "triceps")}>
          ...
          <g className={regionClass(regions, "glutes")}>
          ...
          <g className={regionClass(regions, "hamstrings")}>
  ...
  return (
    <section className={`muscle-exposure${isEmpty ? " is-empty" : ""}`} aria-labelledby="muscle-exposure-title">
      ...
      <p className="muscle-exposure__intro">
        A weekly map of completed, mapped sets from finished or ended sessions. Direct sets count fully and secondary involvement counts half. Active sessions appear only after you finish or end them.
      </p>
      ...
      <div className="muscle-exposure__content">
        <MuscleBodyMap regions={regions} />

        <div className="muscle-exposure__ledger">
          <div className="muscle-exposure__ledger-heading">
            <span>Region ledger</span>
            <small>Weighted set score</small>
          </div>
          <ul>
            {REGION_META.map(({ key, label }) => {
              const region = regions[key];
              const breakdown = region.hasBreakdown
                ? `${formatCount(region.directSets)} direct · ${formatCount(region.secondarySets)} secondary · ${formatCount(region.trainingDays)} ${region.trainingDays === 1 ? "day" : "days"}`
                : `${formatCount(region.score)} relative exposure score`;
              return (
                <li className={`is-${region.tier}`} key={key}>
                  <i aria-hidden="true" />
                  <span>
                    <strong>{label}</strong>
                    <small>{region.tier === "none" ? "No exposure" : `${TIER_LABELS[region.tier]} exposure`}</small>
                  </span>
      ...
      <p className="muscle-exposure__method">
        Relative exposure score describes where recorded work was assigned; it is not a completed-set total. It does not measure load, repetitions, effort, technique, recovery, progressive overload, or muscle growth.
      </p>
